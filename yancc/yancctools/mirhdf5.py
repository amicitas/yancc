...
None
  ...
  HDF5 attributes to allow proper restoration in python.
...
Note:
  ...
  dictionaries in python 3.6. However, key order saving is likely still
  necessary based on how h5py returns entries. I have not checked recent
  behavior to see if this is still the case.

"""

__version__="1.1.1"
__version_date__ = "2025-12-16"
...
def _addDictToHdf5(group, dick, include=None, exclude=None, **kwargs):

    key_list = [key for key in dick.keys() if _keyIncluded(key, include, exclude)]
    key_list_encoded = [key.encode() for key in key_list]
    group.attrs['_mirhdf5 python object type'] = 'dict'.encode()
    try:
        group.attrs['_mirhdf5 dictionary order'] = key_list_encoded
    except TypeError:
...
def _addHdf5ToDict(
    ...
    if group_type != 'dict' and group_type != 'OrderedDict':
...
def _createNewItemFromHdf5(group, key=None, include=None, exclude=None):
    ...
    if isinstance(data, h5py.Group):
        ...
        if group_type == 'dict' or group_type == 'OrderedDict':
            new_item = dict()
...
def _keyIncluded(key, include, exclude):
    """
    Check to see if a key is included or excluded based on include/exclude lists.
    If include and exclude are both None, then simply return True.
    """
    status = True
    if exclude:
...
def convertBytesToStr(dick):
    """
    Conveniance function to recursively convert bytes to str.
    
    This function will only look at dict values, and not iterate
    over lists.
    """
    if isinstance(dick, dict):
        for key in dick:
            if isinstance(dick[key], dict):
                dick[key] = convertBytesToStr(dick[key])
            elif hasattr(dick[key], 'decode'):
                dick[key] = dick[key].decode()
    else:
        if hasattr(dick, 'decode'):
            dick = dick.decode
