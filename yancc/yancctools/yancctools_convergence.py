...
from yancc.yancctools.yancctools import generate_runid, _merge_options
    ...
    for param_name, param_values in params.items():
        ...
        for val in param_values:
            ...
            try:
                # 1. Reconstruct Field
                if eq_type == "desc":
                    field = Field.from_desc(eq_data, rho, current_res["nt"], current_res["nz"])
                elif eq_type == "vmec":
                    field = Field.from_vmec(eq_data, rho**2, current_res["nt"], current_res["nz"])
                elif eq_type == "booz_xform":
                    field = Field.from_booz_xform(eq_data, rho**2, current_res["nt"], current_res["nz"])
                elif eq_type == "ipp_bc":
                    field = Field.from_ipp_bc(eq_data, rho**2, current_res["nt"], current_res["nz"])
                else:
                ...
                _, _, fluxes, _ = solve_dke(
                    field, pitchgrid, speedgrid, local_species, erho, print_every=0, **dke_opts
                )
                ...
                logger.error(f"Failed convergence scan point {param_name}={val}: {e}")
